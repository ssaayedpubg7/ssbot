...
stopuser = {}
token = "6345121817:AAE1anpomUgYjus4gV4KGq16JcL1QRyNsAA"
	...
	def my_function():
	  ...
	  "plan": "Free - Not Subscribed",
		...
		if BL == 'Free - Not Subscribed':	
			...
			contact_button = types.InlineKeyboardButton(text="ϟ Owner - S A Y E D", url="https://t.me/S_0_5_5")
			keyboard.add(contact_button)
			random_number = random.randint(4, 17)
			photo_url = f'https://t.me/S_0_5_5/{random_number}'
...
ϟ Owner ~ @S_0_5_5 </b>''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="ϟ My Telegram ϟ", url="https://t.me/S_0_5_5")
		keyboard.add(contact_button)
...
ϟ - Owner • @S_0_5_5 ϟ</strong>''',reply_markup=keyboard)
...
- Owner ~ @S_0_5_5 - @S_0_5_5 </b>''',reply_markup=keyboard)
			return
		with open('data.json', 'r') as file:
			json_data = json.load(file)
...
ϟ - Owner -> @S_0_5_5⚡</b>'''
                    ...
                    if "Funds" in last or 'avs' in last or 'added' in last or 'Duplicate' in last or 'Approved' in last:
                        live += 1
                        bot.send_message(call.from_user.id, msg)                    
                    if 'Invalid postal' in last or 'CVV' in last:
                        dd += 1
                    else:
                        dd += 1
...
You are Not Subscribed S A Y E D Chk BOT !❌
...
def qwwe(message):
    if str(message.from_user.id) in myid:
        ...
        try:
            ...
            bot.reply_to(message, f'''- Hello S A Y E D
• Done Send - {mp}
• Bad Send - {erop}''')
...
def qwwem(message):
    if str(message.chat.id) in admins:
        user_id = message.text.replace("/del ", "")
...
- Total Bot Users ¦ /tot
- Send Msg Forr All ¦ /msg + msg
- delete A Subsc ¦ /del + ID
- Show Sub's ID's ¦ /sh
...
def generate_code(message):
    def my_function():
        ...
        try:
            ...
            pas = 'SSAAYEDVIP-' + '2024' +'-' + ''.join(random.choices(characters, k=4))
            current_time = datetime.now()
            ...
            msg = f'''<b>ϟ |SS|CC|COMBO|CHECKER| Code
